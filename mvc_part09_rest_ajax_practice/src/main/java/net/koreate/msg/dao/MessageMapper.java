...
public interface MessageMapper {
	...
	/**
	 * message 등록
	 * @param vo - 등록할 메세지 정보 <br>
	 * 			   tbl_message table 에 targetid, sender, message column 에 정보를 추가하여
	 * 			   행정보 삽입		
	 */
	@Insert("INSERT INTO tbl_message(targetid, sender, message) "
			+ "VALUES(#{targetid}, #{sender}, #{message})")
	int create(MessageVO vo) throws Exception;

	/**
	 * @param mno - 수신 확인할 메세지 번호 사용자가 메세지 수신 확인하면 
	 *              opendate column 정보를 수신 확인한 현재시간으로 수정
	 */
	@Update("UPDATE tbl_message SET opendate = CURRENT_TIMESTAMP WHERE mno = #{mno}")
